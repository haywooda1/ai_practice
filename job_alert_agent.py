...
- U.S. Navy veteran — SATCOM, electronics, leadership
...
def score_config(total):
    """Return (hex_color, label, bg_color) based on score."""
    if total >= 80:
...
def build_job_card(job, scoring, rank):
    ...
    text_color, _, card_bg, badge_bg, badge_text = score_config(total)

    # Strong matches get a highlighted left border accent
    is_strong = total >= 80
    ...
    if concerns and concerns.strip().lower() not in ("none", "none."):
        ...
        <div style="display:flex;align-items:flex-start;gap:6px;margin-top:8px;
            ...
            <span style="font-size:12px;color:#92400e;line-height:1.5;">&#9888;&#xfe0e; {concerns}</span>
        </div>"""
    ...
    <div style="background:{card_bg};border:{card_border};border-left:{left_border};
        ...
        <div style="display:flex;align-items:flex-start;gap:14px;">
            ...
            <div style="flex-shrink:0;text-align:center;min-width:52px;">
                <div style="font-size:26px;font-weight:800;color:{text_color};
        ...
        <div style="margin:14px 0 10px;display:grid;grid-template-columns:1fr 1fr;
                    gap:8px 20px;">
            <div>
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:3px;">
                    ...
                    <span style="font-size:11px;font-weight:600;color:#374151;
                                 width:28px;text-align:right;">{tf}/25</span>
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    <span style="font-size:11px;color:#9ca3af;width:72px;">Skills</span>
                    {score_bar_html(sm)}
                    <span style="font-size:11px;font-weight:600;color:#374151;
                                 width:28px;text-align:right;">{sm}/25</span>
                </div>
            </div>
            <div>
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:3px;">
                    ...
                    {score_bar_html(sl)}
                    <span style="font-size:11px;font-weight:600;color:#374151;
                                 width:28px;text-align:right;">{sl}/25</span>
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    <span style="font-size:11px;color:#9ca3af;width:72px;">Industry</span>
                    {score_bar_html(inf)}
                    <span style="font-size:11px;font-weight:600;color:#374151;
                                 width:28px;text-align:right;">{inf}/25</span>
                </div>
            </div>
        </div>
        ...
        <div style="font-size:13px;color:#374151;line-height:1.6;
                    ...
                    border-left:3px solid #d1d5db;margin-bottom:4px;">
            {why}
        </div>
...
def build_html_digest(scored_jobs, all_count):
    ...
    # Split into strong matches and good matches sections
    strong_cards = ""
    good_cards   = ""
    ...
    if strong_cards:
        strong_section = f"""
        <div style="font-size:11px;font-weight:700;color:#15803d;letter-spacing:0.1em;
                    text-transform:uppercase;margin:24px 0 10px;">
            &#9733; Strong Matches &mdash; {strong} job{"s" if strong != 1 else ""}
        </div>
        {strong_cards}"""
    ...
    if good_cards:
        good_section = f"""
        <div style="font-size:11px;font-weight:700;color:#b45309;letter-spacing:0.1em;
                    text-transform:uppercase;margin:24px 0 10px;">
            Good Matches &mdash; {good} job{"s" if good != 1 else ""}
        </div>
        {good_cards}"""
    ...
    if not scored_jobs:
        ...
        <div style="text-align:center;padding:40px 20px;color:#9ca3af;">
            ...
            <div style="font-size:13px;margin-top:6px;">
                No jobs cleared the score threshold. Check back tomorrow.
            </div>
        </div>"""
...
<title>LinkedIn Job Digest &mdash; {date_str}</title>
</head>
...
<div style="max-width:660px;margin:0 auto;padding:28px 16px 40px;">
    ...
    <div style="background:#ffffff;border:1px solid #e5e7eb;border-radius:10px;
        ...
        <div style="display:flex;gap:0;flex-wrap:wrap;">
            <div style="flex:1;min-width:80px;padding:0 16px 0 0;
            ...
            <div style="flex:1;min-width:80px;padding:0 16px;
                        border-right:1px solid #f3f4f6;">
...
def send_digest(html_content, job_count):
    ...
    msg["Subject"] = (f"💼 Job Digest — {job_count} match{'es' if job_count != 1 else ''} · "
...
def main():
    ...
    # ── Build and send HTML digest (even if 0 matches — so you know it ran)
